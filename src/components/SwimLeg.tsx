...
export default function SwimLeg() {
  return (
    <section id="swim" className="bg-race-dark py-20 px-6">
      <div className="max-w-6xl mx-auto">
        {/* Top accent bar */}
        <div className="h-1 w-16 bg-swim rounded-full mb-10" />
            ...
            <ul className="space-y-2">
              {leg.details.map((detail, i) => (
                <li key={i} className="flex items-start gap-3 text-sm font-body text-race-muted">
                  <span className="mt-1 w-1.5 h-1.5 rounded-full bg-swim flex-shrink-0" />
                  {detail}
                </li>
              ))}
            </ul>
